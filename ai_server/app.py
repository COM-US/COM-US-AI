...
word_csv_path = os.path.join(BASE_DIR, "ksl_dictionary_words.csv")
dict_csv_path = os.path.join(BASE_DIR, "ksl_dictionary_final_merged.csv")

@app.route('/ksl/translate', methods=['POST'])
def translate_to_ksl():
    data = request.get_json()
    if not data or "sentence" not in data:
        return jsonify({"status": 400, "message": "문장이 제공되지 않았습니다."}), 400

    sentence = data["sentence"]
    result = run_pipeline(sentence, word_csv_path, dict_csv_path)

    if "error" in result:
        return jsonify({"status": 500, "message": "KSL 변환 실패", "detail": result["error"]}), 500
