...
3. 부정문 처리:
    - 서술어 뒤에 정확한 수어 부정 표현 사용 (예: [없다], [못하다], [아니다] 등)
    - 이중 부정은 제거
4. 복합문 처리:
...
def build_semantic_prompt(sign_sentence, csv_word_groups):
    csv_data_section = "\n".join([f"- {', '.join(group)}" for group in csv_word_groups])
...
[단어 리스트 제공]
다음은 바꿀 수 있는 단어 그룹이야. 이 목록 안에서만 변환할 수 있어: {csv_data_section}
...
def attach_ksl_links(matched_sentence, dict_csv_path):
    df = pd.read_csv(dict_csv_path)
    word_to_url = {}
    for _, row in df.iterrows():
        for word in str(row['단어']).split("/"):
            word_to_url[word.strip()] = row['수어 동영상 URL (MP4)']
    ...
    for token in tokens:
        ...
        found = next(((w, word_to_url[w]) for w in candidates if w in word_to_url), None)
        if found:
            word, url = found
...
def run_pipeline(sentence, word_csv_path, dict_csv_path):
    ...
    final_output = attach_ksl_links(result2, dict_csv_path)
